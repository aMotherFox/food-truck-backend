...
public class CustomerService {
    ...
    public CustomerDto createNewCustomers(NewCustomerRequestBody newCustomerRequestBody) {
        ...
        if (newCustomerRequestBody.getPassword().equals(newCustomerRequestBody.getConfirmPassword())) {
            ...
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Please enter your first name, between 2-50 characters");
        } if (newCustomerRequestBody.getLastName().length() < 2 || newCustomerRequestBody.getLastName().length() > 50) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Please enter your last name, between 2-50 characters");
        } if (newCustomerRequestBody.getPassword().length() < 2 || newCustomerRequestBody.getPassword().length() > 50) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Please enter a unique password, between 2-50 characters");
        } if (newCustomerRequestBody.getConfirmPassword().length() < 2 || newCustomerRequestBody.getConfirmPassword().length() > 50) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Please re-enter your unique password, between 8-50 characters");
        }
